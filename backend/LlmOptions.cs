...
public class LlmOptions : IValidateOptions<LlmOptions>
{
    ...
    public required string RerankingModel { get; set; }
    ...
    public ValidateOptionsResult Validate(string? name, LlmOptions options)
    {
        ...
        if (string.IsNullOrWhiteSpace(options.RerankingModel))
            failures.Add("RerankingModel cannot be null or whitespace");
